...
export default function PrediccionesEditablesPage() {
  ...
  const cargar = useCallback(async () => {
    ...
    const [predsData, equiposData] = await Promise.all([
      supabase
        .from('predicciones')
        .select('*')
        .eq('user_id', user.id),
  ...
  const completadas = partidos.filter((p) => {
  ...
  const faltantes = partidos.length - completadas
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <p className="text-white font-bold">
              Progreso: {completadas}/{partidos.length}
            </p>
          ...
          <div>
            ...
            <p className="text-white/35 text-xs">
              Cada partido cierra a la hora de inicio.
            </p>
          </div>
        </div>
      ...
      {fasesPresentes(partidos).map((fase) => (
        <div key={fase.key} className="mb-6">
          ...
          {partidos.filter((p) => (p.fase || '16avos') === fase.key).map((partido) => (
            <DieciseisavosCard
              key={partido.id}
              partido={partido}
